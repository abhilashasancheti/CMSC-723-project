...
class Saver(object):
    ...
    def save(self, save_dict, step, bleu, rouge1, rouge2, loss):
        ...
        with open(os.path.join(self.model_path, "log"), "a", encoding="UTF-8") as log:
            log.write("%s\t step: %6d\t loss: %.2f\t bleu: %.2f\trouge1: %.2f\trouge2: %.2f\n" % (datetime.datetime.now(), step, loss, bleu, rouge1, rouge2))
